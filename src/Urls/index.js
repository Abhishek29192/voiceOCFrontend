export const BaseUrl = "http://3.6.197.151:3057/api/v1/";
// export const BaseUrl = "http://127.0.0.1:8000/api/v1/";
export const fetcemplateData = `${BaseUrl}template/all`;
export const fetchContacts = `${BaseUrl}contact/`;
export const sendBroadcastDetails = `${BaseUrl}broadcast/allBroadcast`;
export const sendBroadcastDetailsWithDateTime = `${BaseUrl}broadcast/scheduleBroadcast`;
export const sendTeamInboxData = `${BaseUrl}chat/teamInboxContactDetails`;
export const getContactList = `${BaseUrl}chat/teamInboxContactList`;
export const fetchScheduleBroadcast = `${BaseUrl}broadcast/fetchbroadcast`;
export const fetchSingleChatdataUrl = `${BaseUrl}chat/singleChat`;
export const fetchBroadcastHistoryStatusUrl = `${BaseUrl}broadcast/overallBroadCast`;
export const fetchBroadcastHistoryDataUrl = `${BaseUrl}broadcast/broadcstStatus`;
export const fetctokenUrl = `${BaseUrl}admins/login`;
export const signUpUrl = `${BaseUrl}admins/addAdmin`;
export const contactOptionsUrl = `${BaseUrl}chat/fetchContactList`;
export const sendVideoUrl = `${BaseUrl}chat/uploadFile`;
export const exportExcel = `${BaseUrl}contact/excel`;
export const agentListUrl = `${BaseUrl}admins/agentLists`;
export const selectedAgentUrl = `${BaseUrl}chat/assignAgent`;
export const removeAgentUrl = `${BaseUrl}chat/removeAssignedAgent`;
export const logoutUrl = `${BaseUrl}admins/logOut`;
export const allChatDataUrl = `${BaseUrl}chat/teamInboxChatData`;
export const newMessageUrl = `${BaseUrl}chat/updateMessageStatus`;
export const sendNewMessageTemplate = `${BaseUrl}chat/sendTemplate`
export const deleteContactList = `${BaseUrl}contact/deleteExcel`
export const sortBroadcastDataUrl = `${BaseUrl}broadcast/filterBroadcastData`
export const genterateOtpUrl = `${BaseUrl}admins/forgetPassword-otp`
export const forgotPasswordUrl = `${BaseUrl}admins/forgetPassword`



