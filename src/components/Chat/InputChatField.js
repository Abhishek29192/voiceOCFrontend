...
import SentMessageSound from "../MessageTone/SentMessageSound.mp3"
...
export const InputChatField = ({
  ...
  const ref = useClickOutside(() => setOpenFileUpload(false))
  const refEmoji = useClickOutside(() => setOpenEmoji(false))
  ...
  const handleKeyDown = (e) => {
    ...
    // ENTER
      ...
      setSuggestionsActive(false);
      const recievedNotification = new Audio(SentMessageSound)
      recievedNotification.play().catch((err) => console.log(err, "audio error"))
    }
  };
  ...
  return (
    <form onKeyDown={(e) => handleKeyDown(e)} className="relative">
      <div className="flex absolute bottom-[4rem] right-0">
        {openEmoji && (
          <div className="flex">
            <Picker
              size={1}
      ...
      {(contactDetailData.length == 0 ? chatDataAll[0]?.chatDetail.status : contactDetailData.chatDetail.status) === "active" ? (
        <div>
          <div className="w-full border bg-white p-4  flex items-center rounded relative bottom-[-5px] -z-1  justify-between">
            <div className=" w-[85%] relative bg-slate-200 rounded-xl">
              <InputFieldWithoutCounter
                placeholder={placeholder}
                onChange={(e) => { handleChange(e) }}
                value={typeof message !== "string" ? message.name : message}
                className="w-[85%] bg-slate-200 h-[45px] truncate"
              />
              <div
